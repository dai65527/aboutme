...
- エネルギー工学の知識と持ち前の吸収力にて、新卒1年目にして1人で現場対応を任されるなど、早い段階から戦力に。
- 仕事にやりがいはあったものの、調整業務が大半をしめており、エンジニアとしてのキャリアに不安を持つ。
- エンジニアが企画〜設計〜コーディングまで携われるソフトウェアエンジニアを志望し、独学開始。
...
- 講師、メンター不在の中で自走力とコミニュケーション力を鍛える。
- 現在Lv.5.02（3ヶ月先行して入学した1期生も含め、3月10日現在学内最高レベル）
...
## C++
### コンテナクラス再実装
- STLのコンテナクラスのlist、vector、map、stack、queを実装
...
## JavaScript/TypeScript (Vue.js、React、NodeJS)
### Pokedex
...
## Ruby/Ruby on Rails
- 現在勉強中
- C/C++の基礎があるので新しい言語の習得が容易であるように感じます。
- Webアプリケーション制作予定

## チーム開発
- 42Tokyoのプロジェクトでチーム開発を経験済み(Git/githubを活用。）
- （関係ないかもしれませんが）前職では100億円規模のプロジェクトの1員として働いていました。

# その他自信のある点
### 自走力・キャッチアップ力
- 42Tokyoでは講師、教科書等の仕組みが無い中、上記の内容を4~5ヶ月で習得しました。
- 前職では、素早く業務にキャッチアップし、新卒1年目から1人で現場不具合対応を担当するなど早期に戦力として活躍しました。（通常は早くて3年目）
- 42Tokyoのカリキュラム外でもWeb関係技術を積極的に学習しています。
- 予備校等には通わずに高校、大学受験突破した経験があります。
- 留学経験なく独学でTOEIC950点達成しています。
